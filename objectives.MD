# Disclaimer
This document is not meant as a comprehensive resource for passing the Elastic Certification. Its main goal is to provide a quick cheat sheet of the most important settings for each objective with links to the relevant documentation and pointers to the Elasticsearch Engineer 1/2 course modules. In order to use this cheat sheet efficiently, one must already have a very good hands-on experience with Elasticsearch. The official documentation should only be considered as a safety net during the exam. Every objective should have been repeatedly exercised on many different projects before attempting the certification.
